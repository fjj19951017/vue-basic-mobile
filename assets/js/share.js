...
//初始化新版分享授权
function initNewVersionShare(successCallback,wuid){
	
	//发送请求获取appId和jsApiTicket
	var url = '{{jsTicketUrl}}';
    axios.get(url).then(res => {
        var appId=res.data.model.appId;//在jsp页面定义
			var jsApiTicket=res.data.model.jsapi_ticket;//在jsp页面定义
			var timestamp=new Date().getTime()+'';
			var nonceStr=timestamp+parseInt(Math.random()*100000)+'';
			var strSHA1=hex_sha1("jsapi_ticket="+jsApiTicket+"&noncestr="+nonceStr+"&timestamp="+timestamp+"&url="+window.location.href.split('#')[0]);//SHA1加密
        	wx.config({
        		debug: false,
        	    appId: appId,
        	    timestamp: timestamp,
...
export function initShareInfo(shareTitle, shareSummary, shareImgUrl, shareUrl,successCallback,wuid) {
    ...
    try{
        ...
        if(newVersionInitFinish){
        	...
        	initNewVersionShare(successCallback,wuid);
        }
    }catch(e){
    	//do nothing
    }    
